...
class Chat {
    ...
    /*============================ SCHEDULER ============================*/
    // Save new schedule message with it's relavent type to database
    function saveNewScheduleMessage($sender, $message_content, $to_user=Null, $to_group=Null, $to_room=Null, $schedule_date = Null){
        $message_content  = trim($message_content);
        $message_id = null;
        $time = "";
        $preview = null;
        ...
        if(empty($sender)){
            ...
            if($to_user){
                ...
                //active user chat meta
                $to_user_chat_meta_data = $this->getChatMetaData($sender, $to_user, $to_room);
                $last_sent_msg_index = $this->getLastSentMsgIndex($sender, $to_user, $to_room);
                $data = Array ("user_1" => $user_1,
                               "user_2" => $user_2,
                               "sender_id" => $sender,
                               "room_id" => $to_room,
                               "message" => $message_content,
                               "chat_meta_id" => $to_user_chat_meta_data['id'],                         
                               "time" => $schedule_date,
                               "updated_at" => app('db')->now(),
                               "last_sent_msg_index" => $last_sent_msg_index+1,
                            );
                $id = app('db')->insert ('schedule_chats', $data);
            }
            if($id){
                $status = true;
    ...
    //Get current user's schedule messages
    function getScheduleMessages($room_id, $sender, $user_id) {
        ...
        $schedule_messages = app('db')->get('schedule_chats');
...
/*============================ /SCHEDULER ============================*/
    ...
    /*============================ SCHEDULER ============================*/
    // Get last sent scheduled message index.
    function getLastSentMsgIndex($from_user, $to_user, $to_room=false){        
        $sql = "SELECT 
                    max(last_sent_msg_index) as msg_index
                FROM 
                    cn_schedule_chats
                WHERE
                    user_1 = $from_user and
                    user_2 = $to_user and 
                    room_id = $to_room";
        $max_sent_msg_index = app('db')->rawQueryOne($sql);
        return $max_sent_msg_index ? $max_sent_msg_index['msg_index'] : 0;
    }
/*============================ /SCHEDULER ============================*/
