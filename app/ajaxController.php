...
class ajaxController{
    ...
    /*============================ SCHEDULER ============================*/
    // Saving schedule messages
    public function save_schedule_message(){
        $save_msg = true;
        $post_data = app('request')->body;
        $profanity_found = false;

        $msg_contents = $post_data['message_content'];
        $msg_contents = str_replace("\n", "<br/>",$msg_contents);
        $schedule_dates = $post_data['schedule_date'];
        for($i = 0; $i < count($schedule_dates); $i++) { 
            preg_match('/\b(?:(?:https?):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i', $msg_contents, $message_links);
            if (!empty($message_links[0])) {
                $allowed_link =  true;
                if (isset(SETTINGS['domain_filter']) && SETTINGS['domain_filter'] == true) {
                    if (checkLinkValidity($message_links[0])==false) {
                ...
                if ($allowed_link) {
                    ...
                    $url_data['url'] = $message_links[0];
                    $link_message_content = array();
                    $link_message_content['message'] = app('purify')->xss_clean($msg_contents);
                    $link_message_content = array_merge($link_message_content,$url_data);
                    $msg_contents = json_encode($link_message_content,JSON_UNESCAPED_UNICODE);                
                }else{
                    $msg_contents = __('Blocked Domain, Message Not Sent.');
                    $profanity_found = true;
                    $save_msg = false;
                }
            }
            if($save_msg) {                    
                $chat_save = app('chat')->saveNewScheduleMessage(
                    app('auth')->user()['id'],
                    $msg_contents,
                    app('purify')->xss_clean(clean($post_data['active_user'])),
                    app('purify')->xss_clean(clean($post_data['active_group'])),
                    app('purify')->xss_clean(clean($post_data['active_room'])),            
                    app('purify')->xss_clean(clean($schedule_dates[$i]))
                ); 
            } else {
                $chat_save = array();
                $chat_save['status'] = $status;
                $chat_save['message'] = $msg_content;
            }
        }             
        return json_response($chat_save);
    }

    // Load current user's schedule messages
    public function get_schedule_messages(){        
        $post_data = app('request')->body;
        $currnet_room_id = clean($post_data['room_id']);
        $current_user_id = clean($post_data['current_user_id']);

        $shcedule_messages = app('chat')->getScheduleMessages(
            $currnet_room_id,
            app('auth')->user()['id'],
            $current_user_id            
        );

        if(count($shcedule_messages) > 0) {
            $rest = explode("<br/>", $shcedule_messages[0]['message']);            
            for($i = 0 ; $i < count($shcedule_messages) ; $i++) {
                $message_list = explode("<br/>", $shcedule_messages[$i]['message']);          
                $msg_index = ($shcedule_messages[$i]['last_sent_msg_index'] - 1) % count($message_list);
                $message = $message_list[$msg_index];                
                $shcedule_messages[$i]['message'] = $message;
            }
        }

        $result = array();
        $result['schedule_messages'] = $shcedule_messages;
        $result['rest_messages'] = $rest;
        return json_response($result);
    }
    
    // delete current user's schedule messages
    public function delete_schedule_messages(){        
        $post_data = app('request')->body;
        $currnet_room_id = clean($post_data['room_id']);
        $current_user_id = clean($post_data['current_user_id']);

        $shcedule_messages = app('chat')->deleteScheduleMessages(
            $currnet_room_id,
            app('auth')->user()['id'],
            $current_user_id            
        );
